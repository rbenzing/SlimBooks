...
export const up = async (db: IDatabase): Promise<void> => {
  ...
  try {
    const columns = await getColumnNames(db, 'expenses');
    ...
    await db.executeQuery('CREATE INDEX IF NOT EXISTS idx_expenses_status ON expenses (status)');
...
export const down = async (db: IDatabase): Promise<void> => {
  ...
  if (columns.includes('status')) {
    await db.executeQuery('DROP INDEX IF EXISTS idx_expenses_status');
    await db.executeQuery('ALTER TABLE expenses DROP COLUMN status');
  }
};
